...
    useEffect(() => {
        if (filter.length === 0) {
            const filteredPosts = posts.filter(post => post.type === 'Image');
...
        if (filter.length > 0) {
            const filteredPosts = posts.filter(post => post.type === 'Image' && filter.includes(post.party));
            const moreFilteredPosts = filteredPosts.slice(visiblePosts.length, visiblePosts.length + 15); 

            setVisiblePosts(prev => [...prev, ...moreFilteredPosts]);
            setHasMore(visiblePosts.length + moreFilteredPosts.length < filteredPosts.length);
        } else {
            const filteredPosts = posts.filter(post => post.type === 'Image');
            const morePosts = filteredPosts.slice(visiblePosts.length, visiblePosts.length + 15); 

            setVisiblePosts(prev => [...prev, ...morePosts]);
            setHasMore(visiblePosts.length + morePosts.length < filteredPosts.length);
        } 
