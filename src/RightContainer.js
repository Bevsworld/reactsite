...
                    }
                } else {
                    const morePosts = posts.filter(post => post.type === 'Image').slice(visiblePosts.length, visiblePosts.length + 100);
...
        setVisiblePosts((prevVisiblePosts) => prevVisiblePosts.filter((p) => p.id !== post.id));  // Assuming 'id' is unique
    }; 
